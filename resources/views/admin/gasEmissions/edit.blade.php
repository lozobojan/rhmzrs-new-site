...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.gasEmission.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.gas-emissions.update", [$gasEmission->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\GasEmission::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $gasEmission->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gas') ? 'is-invalid' : '' }}" type="text" name="gas" id="gas" value="{{ old('gas', $gasEmission->gas) }}" required>
                @if($errors->has('gas'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="number" name="year" id="year" value="{{ old('year', $gasEmission->year) }}" step="1" required>
                @if($errors->has('year'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" type="text" name="value" id="value" value="{{ old('value', $gasEmission->value) }}" required>
                @if($errors->has('value'))
