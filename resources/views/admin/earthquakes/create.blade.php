...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.earthquakes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="batch_version">{{ trans('cruds.earthquake.fields.batch_version') }}</label>
                <input class="form-control {{ $errors->has('batch_version') ? 'is-invalid' : '' }}" type="text" name="batch_version" id="batch_version" value="{{ old('batch_version', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('earthquake_type') ? 'is-invalid' : '' }}" name="earthquake_type" id="earthquake_type" required>
                    ...
                    @foreach(App\Models\Earthquake::EARTHQUAKE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('earthquake_type', 'AUTOMATIC') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('publish_status') ? 'is-invalid' : '' }}" name="publish_status" id="publish_status">
                    ...
                    @foreach(App\Models\Earthquake::PUBLISH_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('publish_status', 'DRAFT') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('earthquake_date') ? 'is-invalid' : '' }}" type="text" name="earthquake_date" id="earthquake_date" value="{{ old('earthquake_date') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('earthquake_time') ? 'is-invalid' : '' }}" type="text" name="earthquake_time" id="earthquake_time" value="{{ old('earthquake_time') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('magnitude') ? 'is-invalid' : '' }}" type="text" name="magnitude" id="magnitude" value="{{ old('magnitude', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('municipality') ? 'is-invalid' : '' }}" type="text" name="municipality" id="municipality" value="{{ old('municipality', '') }}">
