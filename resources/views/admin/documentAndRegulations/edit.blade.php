...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.documentAndRegulation.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.public-competitions.update", [$documentAndRegulation->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $documentAndRegulation->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('page') ? 'is-invalid' : '' }}" name="page_id" id="page_id" required>
                    @foreach($pages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('page_id') ? old('page_id') : $documentAndRegulation->page->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('page'))
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                data.append('crud_id', '{{ $documentAndRegulation->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
...
Dropzone.options.attachmentsDropzone = {
    ...
    init: function () {
@if(isset($documentAndRegulation) && $documentAndRegulation->attachments)
          var files =
            {!! json_encode($documentAndRegulation->attachments) !!}
              for (var i in files) {
              var file = files[i]
