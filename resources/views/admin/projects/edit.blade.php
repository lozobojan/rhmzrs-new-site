    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="html_content">{{ trans('cruds.project.fields.html_content') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('html_content') ? 'is-invalid' : '' }}" name="html_content" id="html_content">{!! old('html_content', $project->html_content) !!}</textarea>
                @if($errors->has('html_content'))
                    <span class="text-danger">{{ $errors->first('html_content') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.html_content_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="cover_photo">{{ trans('cruds.project.fields.cover_photo') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.cover_photo_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="page_id">{{ trans('cruds.project.fields.page') }}</label>
                <select class="form-control select2 {{ $errors->has('page') ? 'is-invalid' : '' }}" name="page_id" id="page_id" required>
                    @foreach($pages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('page_id') ? old('page_id') : $project->page->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('page'))
                    <span class="text-danger">{{ $errors->first('page') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.page_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($project) && $project->cover_photo)
      var file = {!! json_encode($project->cover_photo) !!}
