...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.seismicStation.title_singular') }}
        </div>
{{--        {{ dd($seismicStation) }}--}}

        <div class="card-body">
            <form method="POST" action="{{ route("admin.seismic-station.update", [$seismicStation->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required" for="station_id">{{ trans('cruds.seismicStation.fields.station_id') }}</label>
                    <input class="form-control {{ $errors->has('station_id') ? 'is-invalid' : '' }}" type="text" name="station_id" id="station_id" value="{{ old('station_id', $seismicStation->station_id) }}" step="1" required>
                ...
                <div class="form-group">
                    ...
                    @if($errors->has('batch_version'))
                        <span class="text-danger">{{ $errors->first('batc_version') }}</span>
                    @endif
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('station_name') ? 'is-invalid' : '' }}" type="text" name="station_name" id="station_name" value="{{ old('station_name', $seismicStation->station_name) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('alt') ? 'is-invalid' : '' }}" type="text" name="alt" id="alt" value="{{ old('alt', $seismicStation->alt) }}" step="1" required>
                    @if($errors->has('alt'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('lng') ? 'is-invalid' : '' }}" type="text" name="lng" id="lng" value="{{ old('lng', $seismicStation->lng) }}" step="1" required>
                    @if($errors->has('lng'))
                ...
                <div class="form-group">
                    <label class="required" for="lat">{{ trans('cruds.seismicStation.fields.lat') }}</label>
                    <input class="form-control {{ $errors->has('lat') ? 'is-invalid' : '' }}" type="text" name="lat" id="lat" value="{{ old('lat', $seismicStation->lat) }}" required>
                    @if($errors->has('lat'))
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $seismicStation->description) !!}</textarea>
